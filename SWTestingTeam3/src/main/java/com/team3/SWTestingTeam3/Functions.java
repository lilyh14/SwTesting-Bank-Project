...
public class Functions {
  ...
  // Credit Card Payoff
  static ArrayList<Double> option1(double bal, double interest, double payoff) {
    // Credit Card Payoff
    ...
    double payment, ppaid, interestpaid = 0;
    ...
    results.add((payment * 100 / 100.0d));
    results.add((ppaid * 100 / 100.0d));
    results.add((interestpaid * 100 / 100.0d));

    return results;
  }

  // Simple Savings Calculator
  static ArrayList<Float> option2(
      float initDeposit,
      float monthlyContribution,
      float years,
      float interest) { // Simple Savings Calculator
    ...
    // For each compounding period, FORMULA: currBalance = (currBalance + contributions during the
    // period) + (1 + interestRate)
    while (periodsRemaining > 0) {
      if (periodsRemaining > 0
          && periodsRemaining
              < 1) // if years has a .5, need to take half its yearlyContributions and half APY
                   // interest
      {
        factor = 0.5f;
      } else {
  ...
  static ArrayList<Float> option3(
      float creditCardBalance, float creditCardInterestRate, float minPaymentPercentage) {
    ...
    float totalAmountPaid =
        creditCardBalance + ((creditCardInterestRate / 100) * creditCardBalance);
  ...
  static ArrayList<Float> option4(float deposit, float period, float apy) {
    ...
    float total =
        Float.parseFloat(df.format(deposit * (float) Math.pow((1 + (apy / 100)), period)));
    float interest = Float.parseFloat(df.format(total - deposit));

    System.out.println("Total Balance: $" + total);
    System.out.println("Interest Earned: $" + total);
    ...
    while (true) {
      ...
      switch (option) {
        case 1:
          ...
          while (true) {
            ...
            try {
              ...
              System.out.println(
                  "Invalid input. Input balance must be numerical and greater than 0.");
              input.nextLine();
            }
          }
          ...
          while (true) {
            ...
            try {
              ...
              System.out.println(
                  "Invalid input. Input balance must be numerical and between 0-100.");
              input.nextLine();
            }
          }
          ...
          while (true) {
            ...
            try {
              payoff = input.nextDouble();
              if (payoff < 1) throw new Exception();
              break;
            } catch (Exception e) {
              System.out.println(
                  "Invalid input. Input balance must be numerical and greater than or equal to 1.");
              input.nextLine();
            }
          }
        ...
        case 2:
          ...
          System.out.println(
              "Please enter period in years (can enter .5 years): "); // validation a bit different
          do {
            try {
              ...
              if (years % 0.5 == 0) // check to see if divisible by 0.5
              {
                ...
                System.out.println(
                    "Period (years) must be a factor of 0.5! Ex: 3 or 2.5 or 3.0. Try again.");
                valid = false;
              }
              if (years <= 0) {
        ...
        case 3:
          ...
          while (true) {
            ...
            if (userInput < 0.00 || userInput > 100.00) {
              System.out.println(
                  "Minimum payment percentage must be between 0.0 and 100.0. Please try again.");
              continue;
            } else {
        ...
        case 4:
          ...
          System.out.print("Please enter an initial deposit in US Dollars ($): ");
          while (true) {
            try {
              ...
              System.out.print(
                  "Invalid input! Please enter an initial deposit in US Dollars ($) with only numerical characters: ");
              input.nextLine();
            } catch (Exception e) {
              System.out.print(
                  "Invalid input! Please enter an initial deposit in US Dollars ($) that is non-negative: ");
              input.nextLine();
            }
          }
          ...
          while (true) {
            try {
              ...
              if (period < 0 || (Math.floor(period) != period)) {
                throw new Exception();
              }
              break;
            } catch (InputMismatchException e) {
              System.out.print(
                  "Invalid input! Please enter a period in years with only numerical characters: ");
              input.nextLine();
            } catch (Exception e) {
              System.out.print(
                  "Invalid input! Please enter a period in years as a positive integer: ");
              input.nextLine();
            }
          }
          System.out.print("Please enter an Annual Percentage Yield as a percent (%): ");
          while (true) {
            try {
              apy = input.nextFloat();
              if (apy > 100 || apy < 0) {
              ...
              System.out.print(
                  "Invalid input! Please enter an Annual Percentage Yield as a percent (%) with only numerical characters: ");
              input.nextLine();
            } catch (Exception e) {
              System.out.print(
                  "Invalid input! Please enter an Annual Percentage Yield as a percent (%) with values ranging only from 0 to 100: ");
              input.nextLine();
            }
          }
